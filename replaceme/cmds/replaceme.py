...
from shamrock.cmds.configure import configure_cmd
from shamrock.cmds.farm import farm_cmd
from shamrock.cmds.init import init_cmd
from shamrock.cmds.keys import keys_cmd
from shamrock.cmds.netspace import netspace_cmd
from shamrock.cmds.passphrase import passphrase_cmd
from shamrock.cmds.plots import plots_cmd
from shamrock.cmds.show import show_cmd
from shamrock.cmds.start import start_cmd
from shamrock.cmds.stop import stop_cmd
from shamrock.cmds.wallet import wallet_cmd
from shamrock.cmds.plotnft import plotnft_cmd
from shamrock.cmds.plotters import plotters_cmd
from shamrock.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
...
from shamrock.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage shamrock blockchain infrastructure ({__version__})\n",
    epilog="Try 'shamrock start node', 'shamrock netspace -d 192', or 'shamrock show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from shamrock.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
if not supports_keyring_passphrase():
    from shamrock.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show shamrock version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs shamrock daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by shamrock start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from shamrock.daemon.server import async_run_daemon
