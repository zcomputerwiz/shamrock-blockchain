...
from shamrock.daemon.server import singleton, service_launch_lock_path
from shamrock.server.ssl_context import shamrock_ssl_ca_paths, private_ssl_ca_paths
...
class Service:
    def __init__(
        ...
        proctitle_name = f"shamrock_{service_name}"
        ...
        shamrock_ca_crt, shamrock_ca_key = shamrock_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = ShamrockServer(
            advertised_port,
            node,
            peer_api,
            node_type,
            ...
            (shamrock_ca_crt, shamrock_ca_key),
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for ShamrockServer to be closed")
