...
from shamrock.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from shamrock.plotters.chiapos import get_chiapos_install_info, plot_shamrock
from shamrock.plotters.madmax import get_madmax_install_info, plot_madmax
from shamrock.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in SHAMROCK_ROOT.
    shamrock_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within SHAMROCK_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    if args.plotter == "chiapos":
        plot_shamrock(args, shamrock_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, shamrock_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, shamrock_root_path, root_path)
