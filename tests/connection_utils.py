...
async def disconnect_all_and_reconnect(server: ShamrockServer, reconnect_to: ShamrockServer) -> bool:
...
async def add_dummy_connection(
    server: ShamrockServer, dummy_port: int, type: NodeType = NodeType.FULL_NODE
    ...
    generate_ca_signed_cert(
        server.shamrock_ca_crt_path.read_bytes(), server.shamrock_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.shamrock_ca_crt_path, server.shamrock_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: ShamrockServer, server_2: ShamrockServer) -> WSShamrockConnection:
    """
