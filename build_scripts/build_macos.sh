...
# sets the version for shamrock-blockchain in package.json
brew install jq
cp package.json package.json.orig
jq --arg VER "$SHAMROCK_INSTALLER_VERSION" '.version=$VER' package.json > temp.json && mv temp.json package.json
...
if [ "$NOTARIZE" == true ]; then
  electron-osx-sign Shamrock-darwin-x64/Shamrock.app --platform=darwin \
...
mv Shamrock-darwin-x64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="Shamrock-$SHAMROCK_INSTALLER_VERSION.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/Shamrock-darwin-x64/Shamrock.app Shamrock-$SHAMROCK_INSTALLER_VERSION \
