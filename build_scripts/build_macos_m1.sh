...
electron-packager . Replaceme --asar.unpack="**/daemon/**" --platform=darwin \
...
if [ "$NOTARIZE" ]; then
  electron-osx-sign Replaceme-darwin-arm64/Replaceme.app --platform=darwin \
  --hardened-runtime=true --provisioning-profile=replacemeblockchain.provisionprofile \
...
mv Replaceme-darwin-arm64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="Replaceme-$REPLACEME_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/Replaceme-darwin-arm64/Replaceme.app Replaceme-$REPLACEME_INSTALLER_VERSION-arm64 \
...
if [ "$NOTARIZE" ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.replaceme.blockchain \
...
# xcrun altool --notarize-app -f Replaceme-0.1.X.dmg --primary-bundle-id net.replaceme.blockchain -u username -p password
...
# xcrun stapler staple Replaceme-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate Replaceme-0.1.X.dmg
