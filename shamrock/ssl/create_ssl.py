...
from shamrock.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def get_shamrock_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "shamrock_ca.crt")
    key = pkg_resources.resource_string(__name__, "shamrock_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("shamrock.network")]),
            critical=False,
        )
...
def main():
    return make_ca_cert(Path("./shamrock_ca.crt"), Path("./shamrock_ca.key"))
