...
ROOT = pathlib.Path(importlib.import_module("shamrock").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `shamrock` file name (which is the executable)
    and the `shamrock` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `shamrock` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("shamrock/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("shamrock-blockchain"))[0]
...
# TODO: collapse all these entry points into one `shamrock_exec` entrypoint that accepts the server as a parameter

entry_points = ["shamrock.cmds.shamrock"] + [f"shamrock.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "shamrock.util.bip39"])

if THIS_IS_WINDOWS:
    shamrock_mod = importlib.import_module("shamrock")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/shamrock/util/english.txt", "shamrock/util"))
datas.append((f"{ROOT}/shamrock/util/initial-config.yaml", "shamrock/util"))
datas.append((f"{ROOT}/shamrock/wallet/puzzles/*.hex", "shamrock/wallet/puzzles"))
datas.append((f"{ROOT}/shamrock/ssl/*", "shamrock/ssl"))
...
add_binary("shamrock", f"{ROOT}/shamrock/cmds/shamrock.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/shamrock/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/shamrock/server/start_{server}.py", COLLECT_ARGS)
