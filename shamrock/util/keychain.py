...
DEFAULT_USER = f"user-shamrock-{CURRENT_KEY_VERSION}"  # e.g. user-shamrock-1.8
DEFAULT_SERVICE = f"shamrock-{DEFAULT_USER}"  # e.g. shamrock-user-shamrock-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting SHAMROCK_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("SHAMROCK_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from shamrock.cmds.passphrase_funcs import prompt_for_passphrase
